...
<aside
	class="fixed inset-y-0 left-0 z-40 flex flex-col bg-white transition-all duration-200
		...
		{#if !collapsed}
			...
			<div class="rounded-xl p-3.5" style="background-color: #f7f6f3; border: 1px solid rgba(45, 45, 42, 0.06);">
				<div class="flex items-center gap-1.5 text-xs" style="color: #6b6b65;">
					<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="size-3.5 shrink-0" aria-hidden="true">
						<path stroke-linecap="round" stroke-linejoin="round" d="M12 21a9.004 9.004 0 0 0 8.716-6.747M12 21a9.004 9.004 0 0 1-8.716-6.747M12 21c2.485 0 4.5-4.03 4.5-9S14.485 3 12 3m0 18c-2.485 0-4.5-4.03-4.5-9S9.515 3 12 3m0 0a8.997 8.997 0 0 1 7.843 4.582M12 3a8.997 8.997 0 0 0-7.843 4.582m15.686 0A11.953 11.953 0 0 1 12 10.5c-2.998 0-5.74-1.1-7.843-2.918m15.686 0A8.959 8.959 0 0 1 21 12c0 .778-.099 1.533-.284 2.253m0 0A17.919 17.919 0 0 1 12 16.5c-3.162 0-6.133-.815-8.716-2.247m0 0A9.015 9.015 0 0 1 3 12c0-1.605.42-3.113 1.157-4.418" />
					</svg>
					<span class="font-medium" style="color: #1a1a17;">Chrome Extension</span>
				</div>
				<p class="mt-1 text-xs" style="color: #6b6b65;">Add items instantly from any page.</p>
			</div>
			...
			<div>
				<div class="flex items-center justify-between text-sm mb-2.5">
					...
					<span class="font-semibold" style="color: #1a1a17;">{itemCount} / {auth.plan.maxItems}</span>
				</div>
				<div class="w-full rounded-full h-2" style="background-color: #f4f3ef;">
					<div
			...
			<button
				type="button"
				class="w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl text-sm font-medium transition-all hover:shadow-md"
		...
		<div class="relative">
			...
			>
				...
				{#if !collapsed}
					...
					<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" class="size-3.5 shrink-0" style="color: #6b6b65;" aria-hidden="true">
				...
				>
					<div class="px-3 py-2.5" style="border-bottom: 1px solid rgba(45, 45, 42, 0.06);">
						<p class="text-xs" style="color: #6b6b65;">
							Plan: <span class="font-medium capitalize" style="color: #1a1a17;">{auth.plan.type}</span>
						</p>
						{#if auth.plan.type === 'free'}
							<p class="mt-0.5 text-xs" style="color: #6b6b65;">Upgrade to Pro (₩6,900/month)</p>
						{/if}
					</div>
					<a href="/settings" onclick={() => (userMenuOpen = false)} class="flex w-full items-center gap-2 px-3 py-2 text-sm transition hover:bg-[#f7f6f3]" style="color: #1a1a17;" role="menuitem">
					...
					<a href="/plan" onclick={() => (userMenuOpen = false)} class="flex w-full items-center gap-2 px-3 py-2 text-sm transition hover:bg-[#f7f6f3]" style="color: #1a1a17;" role="menuitem">
						<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" class="size-3.5" aria-hidden="true">
							<path stroke-linecap="round" stroke-linejoin="round" d="M2.25 8.25h19.5M2.25 9h19.5m-16.5 5.25h6m-6 2.25h3m-3.75 3h15a2.25 2.25 0 0 0 2.25-2.25V6.75A2.25 2.25 0 0 0 19.5 4.5h-15a2.25 2.25 0 0 0-2.25 2.25v10.5A2.25 2.25 0 0 0 4.5 19.5Z" />
						</svg>
						Plan
					...
					<button type="button" onclick={logout} class="flex w-full items-center gap-2 px-3 py-2 text-sm transition hover:bg-[#fee8e8]" style="color: #d4183d;" role="menuitem">
						<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" class="size-3.5" aria-hidden="true">
