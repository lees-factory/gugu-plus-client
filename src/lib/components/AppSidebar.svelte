...
>
	...
	>
		...
		>
			<div
				class="flex size-11 shrink-0 items-center justify-center rounded-2xl text-white shadow-lg shadow-zinc-900/10"
			...
			{#if !collapsed}
				<div class="overflow-hidden">
					<div
						class="text-xl leading-none font-semibold tracking-tight whitespace-nowrap text-zinc-900"
					>
						{t('brand_name')}
					</div>
		...
		{#each navItems as item (item.path)}
			<a
				...
				class="group flex items-center rounded-2xl text-sm font-medium transition-all duration-200
					{collapsed ? 'gap-0 md:justify-center md:px-0 md:py-3' : 'gap-3.5 px-4 py-3'}
					{isActive(item.path)
				...
				<span class="shrink-0">
					{#if item.path === '/'}
						<svg
							...
							class="size-[18px] {isActive(item.path)
								? 'text-stone-800'
	...
	{#if !collapsed}
		<div class="shrink-0 space-y-3 p-4" style="border-top: 1px solid rgba(0, 0, 0, 0.06);">
			...
			<!-- Pro Tip card -->
			<div
				class="rounded-2xl border border-amber-100/50 p-5"
