<script lang="ts">
	...
	const parsedRows = $derived(rows.map((r) => ({ ...r, parsed: parseCommerceProductUrl(r.url) })));
...
{#if open}
	<div class="fixed inset-0 z-[100] flex items-end justify-center sm:items-center sm:p-6">
		<!-- Backdrop -->
		<div class="absolute inset-0 bg-zinc-900/30" role="presentation" onclick={onClose}></div>
		...
		<div
			...
			role="dialog"
			aria-modal="true"
			class="relative z-10 flex max-h-[85svh] w-full flex-col rounded-t-3xl border border-zinc-200/50 bg-white shadow-2xl sm:max-w-lg sm:rounded-3xl"
		>
				...
				<form onsubmit={handleSubmit} class="flex flex-col gap-4 px-8 pt-8 pb-8 md:px-10 md:pb-10">
								...
								{#if row.url.trim()}
									<div class="mt-1.5 flex flex-wrap items-center gap-2 pl-1">
										...
										{:else if row.parsed.displayName}
											...
											>
												<span class="size-1.5 rounded-full" style="background-color: #f43f5e;"
												></span>
												{row.parsed.displayName}
											</span>
					...
					{#if rows.length < MAX_ITEMS}
						...
						>
							...
							{t('modal_add_another')}
							<span
								class="rounded-full bg-zinc-200/80 px-2 py-0.5 text-[11px] font-semibold text-zinc-500"
								>{rows.length}/{MAX_ITEMS}</span
							>
						</button>
					{:else}
						<div
							class="flex items-center justify-center rounded-2xl bg-zinc-100/60 py-3 text-xs font-medium text-zinc-400"
						>
