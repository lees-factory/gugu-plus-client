<script lang="ts">
	...
	$effect(() => {
		if (open) {
			void tick().then(() => urlInputEl?.focus());
		}
	});
...
{#if open}
	<div class="fixed inset-0 z-[100] flex items-end justify-center sm:items-center sm:p-6">
		...
		<!-- Dialog -->
		<div
			class="relative z-10 w-full overflow-hidden rounded-t-3xl border border-zinc-200/50 bg-white/95 shadow-2xl backdrop-blur-xl sm:max-w-lg sm:rounded-3xl"
		>
			<!-- Handle bar (mobile) -->
			...
			<form onsubmit={handleSubmit} class="flex flex-col gap-5 px-8 pt-8 pb-8 md:px-10 md:pb-10">
				...
				<div class="group relative">
					<svg
						...
						stroke="currentColor"
						stroke-width="2"
						class="absolute top-1/2 left-4 size-5 -translate-y-1/2 text-zinc-400 transition-colors group-focus-within:text-stone-700"
						aria-hidden="true"
					>
						<path
					...
					<input
						...
						required
						class="w-full rounded-2xl border-2 border-zinc-200/80 bg-zinc-50/50 py-4 pr-4 pl-12 text-sm font-normal transition-all outline-none placeholder:text-zinc-400 focus:border-stone-400 focus:bg-white"
						style="box-shadow: none;"
						onfocus={(e) => {
							(e.target as HTMLInputElement).style.boxShadow = '0 0 0 4px rgba(41, 37, 36, 0.08)';
						}}
						onblur={(e) => {
				...
				{#if url.trim()}
					<div class="-mt-2 flex flex-wrap items-center gap-2">
						...
						{:else}
							<span
								class="inline-flex items-center rounded-full border border-zinc-200/60 bg-zinc-100 px-2.5 py-1 text-xs font-medium text-zinc-600"
							>
								{parsed.message}
							</span>
						{/if}
