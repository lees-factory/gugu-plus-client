...
<div class="rounded-2xl bg-white p-7 shadow-sm" style="border: 1px solid rgba(45, 45, 42, 0.08);">
	<div class="mb-6">
		...
		<p class="mt-1 text-sm" style="color: #6b6b65;">{t('signup_subtitle')}</p>
	</div>
	...
	>
		<div class="flex flex-col gap-2">
			<label for="display_name" class="text-sm font-medium" style="color: #1a1a17;"
				>{t('auth_name_label')}</label
			>
			<input
				id="display_name"
				name="display_name"
		...
		<div class="flex flex-col gap-2">
			<label for="confirm" class="text-sm font-medium" style="color: #1a1a17;"
				>{t('auth_confirm_label')}</label
			>
			<input
				id="confirm"
				type={showPassword ? 'text' : 'password'}
			...
			/>
			{#if passwordMismatch}
				<p class="text-xs" style="color: #d4183d;">{t('signup_mismatch')}</p>
			{/if}
		</div>
