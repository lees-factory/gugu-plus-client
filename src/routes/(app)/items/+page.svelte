	...
	<div class="flex items-center gap-3">
		...
		<div class="flex flex-1 items-center justify-between">
			...
			>
				...
				<span class="hidden sm:inline">{t('home_add_item')}</span>
				<span class="sm:hidden">{t('home_add_short')}</span>
			</button>
		</div>
	</div>
</div>
...
{#if home.model.loading}
	<div class="flex flex-col items-center justify-center gap-4 px-6 py-20 sm:px-8">
		...
		<p class="text-sm" style="color: #6b6b65;">{t('home_loading')}</p>
	</div>
{:else if !auth.user}
	<div class="flex flex-col items-center justify-center gap-6 px-6 py-24 text-center sm:px-8">
		<div
			class="flex size-16 items-center justify-center rounded-2xl"
			style="background: linear-gradient(135deg, #e8f2f0 0%, #f0f9f7 100%);"
		>
			<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" class="size-8" style="color: #5aad9c;" aria-hidden="true">
				<path stroke-linecap="round" stroke-linejoin="round" d="M20.25 7.5l-.625 10.632a2.25 2.25 0 0 1-2.247 2.118H6.622a2.25 2.25 0 0 1-2.247-2.118L3.75 7.5M10 11.25h4M3.375 7.5h17.25c.621 0 1.125-.504 1.125-1.125v-1.5c0-.621-.504-1.125-1.125-1.125H3.375c-.621 0-1.125.504-1.125 1.125v1.5c0 .621.504 1.125 1.125 1.125Z" />
			</svg>
		</div>
		<div>
			<h2 class="text-xl font-semibold" style="color: #1a1a17;">{t('guest_title')}</h2>
			<p class="mt-2 max-w-sm text-sm" style="color: #6b6b65;">{t('guest_desc')}</p>
		</div>
		<div class="flex gap-3">
			<a
				href={localizeHref('/auth/login')}
				class="rounded-xl px-6 py-2.5 text-sm font-medium text-white transition-all hover:shadow-md"
			...
			<a
				...
				class="rounded-xl px-6 py-2.5 text-sm font-medium transition-all hover:shadow-sm"
				style="background-color: #f7f6f3; color: #1a1a17; border: 1px solid rgba(45,45,42,0.08);"
			>
				{t('guest_signup')}
			</a>
		</div>
	...
	<div class="max-w-[1400px] p-6 sm:p-8 lg:p-10">
		...
		>
			<div class="flex items-center gap-4">
				<div class="flex size-12 shrink-0 items-center justify-center rounded-xl bg-white shadow-sm">
					<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="size-5" style="color: #5aad9c;" aria-hidden="true">
						<path stroke-linecap="round" stroke-linejoin="round" d="M12 21a9.004 9.004 0 0 0 8.716-6.747M12 21a9.004 9.004 0 0 1-8.716-6.747M12 21c2.485 0 4.5-4.03 4.5-9S14.485 3 12 3m0 18c-2.485 0-4.5-4.03-4.5-9S9.515 3 12 3m0 0a8.997 8.997 0 0 1 7.843 4.582M12 3a8.997 8.997 0 0 0-7.843 4.582m15.686 0A11.953 11.953 0 0 1 12 10.5c-2.998 0-5.74-1.1-7.843-2.918m15.686 0A8.959 8.959 0 0 1 21 12c0 .778-.099 1.533-.284 2.253m0 0A17.919 17.919 0 0 1 12 16.5c-3.162 0-6.133-.815-8.716-2.247m0 0A9.015 9.015 0 0 1 3 12c0-1.605.42-3.113 1.157-4.418" />
					</svg>
				</div>
				<div>
					<p class="mb-0.5 font-medium" style="color: #1a1a17;">{t('home_chrome_banner_title')}</p>
					<p class="text-sm" style="color: #6b6b65;">{t('home_chrome_banner_desc')}</p>
				</div>
			</div>
			<button
				type="button"
