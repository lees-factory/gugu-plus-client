...
{:else}
	<!-- Sticky header -->
	<div
		class="sticky top-0 z-10 border-b border-zinc-200/50 bg-white/70 px-6 py-4 backdrop-blur-2xl sm:px-8"
	>
		<a
			href={resolve('/items')}
		...
		<div class="flex items-start justify-between gap-4">
			<div class="min-w-0 flex-1">
				<div class="flex flex-wrap items-center gap-2">
					<h1 class="line-clamp-2 text-base font-semibold tracking-tight text-zinc-900 sm:text-lg">
						{item.title}
					</h1>
					<span
						class="shrink-0 rounded-lg border border-zinc-200/60 bg-zinc-50/80 px-2.5 py-0.5 text-[10px] font-semibold tracking-wider text-zinc-600 uppercase"
					>
						{item.site}
					</span>
				</div>
			...
			>
				...
				<span class="hidden sm:inline"
					>{page.ui.deleting ? t('detail_deleting') : t('detail_delete')}</span
				>
			</button>
		</div>
	</div>
			...
			{#if item.trackedItemId}
				<div class="flex justify-center pt-2">
					<button
						...
						class="inline-flex items-center gap-2 rounded-xl px-4 py-3 text-sm font-medium text-rose-600 transition hover:bg-[#fee8e8] disabled:opacity-40"
					>
						<svg
							viewBox="0 0 24 24"
							fill="none"
