...
{#if !item}
	<div class="flex flex-col items-center justify-center gap-4 p-12 text-center">
		<p class="text-sm text-zinc-500">상품을 찾을 수 없습니다.</p>
		<a href="/" class="text-sm font-medium text-zinc-900 underline underline-offset-2">
			대시보드로 돌아가기
		</a>
	</div>
{:else}
	<!-- Sticky header -->
	<div class="sticky top-0 z-10 border-b border-zinc-200 bg-white px-4 py-3 sm:px-6 sm:py-4">
		<a
			href="/"
			class="mb-3 inline-flex items-center gap-1.5 text-sm text-zinc-500 transition hover:text-zinc-900"
		>
		...
		<div class="flex items-start justify-between gap-4">
			<div class="min-w-0 flex-1">
				<div class="flex flex-wrap items-center gap-2">
					<h1 class="line-clamp-2 text-base font-semibold text-zinc-900 sm:text-lg">{item.title}</h1>
					<span class="shrink-0 rounded-md px-1.5 py-0.5 text-xs font-medium {siteBadgeClass}">
						{item.site}
					</span>
				</div>
				<a
					...
					class="mt-1 inline-flex items-center gap-1 text-xs text-blue-600 transition hover:text-blue-700"
				>
					{item.site}에서 보기
			...
			<button
				...
				class="flex shrink-0 items-center gap-1.5 rounded-lg px-3 py-2 text-sm font-medium text-red-600 transition hover:bg-red-50"
			>
				<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" class="size-4" aria-hidden="true">
	...
	<div class="p-4 sm:p-6 lg:p-8">
		<div class="mx-auto max-w-5xl space-y-4 sm:space-y-6">
			...
			<div class="grid grid-cols-1 gap-4 sm:gap-6 md:grid-cols-2">
				<!-- Image -->
				<div class="flex items-center justify-center overflow-hidden rounded-xl border border-zinc-200 bg-white p-6">
					{#if !imgError && displayImage}
						<img
							src={displayImage}
							alt={item.title}
							class="max-h-72 w-full rounded-lg object-contain sm:max-h-96"
							onerror={() => (imgError = true)}
						/>
					{:else}
						<div class="flex h-48 w-full items-center justify-center rounded-lg bg-zinc-50 sm:h-64">
							<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" class="size-12 text-zinc-200" aria-hidden="true">
				...
				<div class="flex flex-col gap-4">
					...
					<div class="rounded-xl border border-zinc-200 bg-white p-5">
						<p class="mb-1 text-xs font-medium text-zinc-500">현재 가격</p>
						<div class="flex items-end gap-2">
							<p class="text-3xl font-semibold tabular-nums text-zinc-900 sm:text-4xl">
								{formatKRW(displayPrice)}
							</p>
							{#if discountPct}
								<span class="mb-1 rounded-md bg-red-50 px-1.5 py-0.5 text-sm font-semibold text-red-600">
									-{discountPct}%
								</span>
							{/if}
						</div>
						{#if originalPrice && originalPrice > displayPrice}
							<p class="mt-1 text-sm text-zinc-400 line-through tabular-nums">
								{formatKRW(originalPrice)}
							</p>
						{/if}
						{#if originalPrice && originalPrice > displayPrice}
							<div class="mt-3 inline-flex items-center gap-1.5 rounded-lg bg-emerald-50 px-3 py-1.5 text-sm font-medium text-emerald-700">
								<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" class="size-4" aria-hidden="true">
					...
					<div class="rounded-xl border border-zinc-200 bg-white p-5">
						<div class="divide-y divide-zinc-100">
							...
							<div class="flex items-center justify-between py-3 first:pt-0 last:pb-0">
								...
								<span class="text-sm font-medium text-zinc-900">매 {item.trackingFrequency}마다</span>
							</div>
							<div class="flex items-center justify-between py-3 first:pt-0 last:pb-0">
								<div class="flex items-center gap-2 text-sm text-zinc-500">
									<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" class="size-4 shrink-0" aria-hidden="true">
								...
								<button
									...
									class="flex items-center gap-1.5 rounded-md px-2 py-1 text-sm font-medium transition {alertEnabled ? 'bg-zinc-100 text-zinc-900' : 'text-zinc-400 hover:text-zinc-600'}"
								>
			...
			{#if hasColors || hasSizes}
				<div class="rounded-xl border border-zinc-200 bg-white p-5 space-y-5">
					<!-- Color/Design selector -->
					{#if hasColors}
						<div>
							<p class="mb-2.5 text-sm font-medium text-zinc-700">
								디자인
								<span class="ml-1.5 text-xs font-normal text-zinc-400">{colorOptions.length}가지</span>
							</p>
								...
								{#each colorOptions as opt}
									{#if opt.image}
										<button
											...
											title={opt.value}
											class="size-12 overflow-hidden rounded-lg border-2 transition sm:size-14
					...
					{#if hasSizes}
						<div>
							<p class="mb-2.5 text-sm font-medium text-zinc-700">사이즈</p>
							<div class="flex flex-wrap gap-2">
								{#each sizeOptions as size}
									...
									<button
										...
										class="min-w-[3rem] rounded-md border px-3 py-2 text-sm font-medium transition
											{selectedSize === size
												? 'border-zinc-900 bg-zinc-900 text-white'
												: 'border-zinc-200 text-zinc-700 hover:border-zinc-300 hover:bg-zinc-50'}"
									>
					...
					{#if currentSku}
						<div class="rounded-lg bg-zinc-50 px-4 py-3 text-sm">
							<span class="text-zinc-500">선택된 옵션:</span>
			...
			<!-- Price history -->
			<div class="rounded-xl border border-zinc-200 bg-white p-5">
				<h2 class="mb-4 text-base font-semibold text-zinc-900">가격 히스토리</h2>

				<!-- Chart placeholder -->
				<div class="mb-5 flex h-40 items-center justify-center rounded-lg border border-zinc-100 bg-zinc-50 sm:h-56">
					<div class="text-center">
						<div class="mx-auto mb-2 flex size-10 items-center justify-center rounded-lg bg-zinc-200">
							<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" class="size-5 text-zinc-400" aria-hidden="true">
								<path stroke-linecap="round" stroke-linejoin="round" d="M2.25 18 9 11.25l4.286-4.286a11.948 11.948 0 0 1 4.306 6.43l.776 2.898m0 0 3.182-5.511m-3.182 5.51-5.511-3.181" />
							</svg>
						</div>
						<p class="text-xs text-zinc-400">가격 차트가 곧 제공됩니다</p>
					</div>
				</div>

				<!-- History list -->
				<div class="divide-y divide-zinc-100">
					{#each item.priceHistory as entry}
						<div class="flex items-center justify-between py-3">
							<span class="text-sm text-zinc-500">{entry.date}</span>
							<div class="flex items-center gap-4">
								<span class="text-sm font-medium tabular-nums text-zinc-900">{formatKRW(entry.price)}</span>
								{#if entry.change < 0}
									<span class="w-24 text-right text-sm font-medium tabular-nums text-emerald-600">
										−{formatKRW(Math.abs(entry.change))}
									</span>
								{:else if entry.change > 0}
									<span class="w-24 text-right text-sm font-medium tabular-nums text-red-500">
										+{formatKRW(entry.change)}
									</span>
								{:else}
									<span class="w-24 text-right text-sm text-zinc-300">변동 없음</span>
								{/if}
							</div>
						</div>
					{/each}
